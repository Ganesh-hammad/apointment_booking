            ...
            <div className="flex items-center gap-4">
                {
                    token && userData
                        ...
                        : <button onClick={() => navigate("/login")} className='bg-primary  rounded-full text-white px-8 py-3 font-light hidden md:block'>Create Account</button>
                }
                <img onClick={() =>  setShowMenu(true)} className='w-6 md:hidden' src={assets.menu_icon} alt="" />
                {/* -------------- Mobile Menu-------------- */}
                <div className={`${showMenu ? 'fixed w-full': 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-none duration-200 `}>
                    <div className="flex items-center justify-between px-5 py-6 border-b border-b-gray-400 ">
                        <img className='w-36' src={assets.logo} alt="" />
                        <img className='w-7' onClick={()=>{setShowMenu(false)}} src={assets.cross_icon} alt="" />
                    </div>
                    <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                        <NavLink onClick={()=>setShowMenu(false)}  to='/'> <p className='px-4 py-2 rounded inline-block w-full'>HOME</p> </NavLink>
